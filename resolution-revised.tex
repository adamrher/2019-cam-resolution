...
{\color{red}{Conventional physics packages interact with the adiabatic dynamics to produce non- or weakly- converging solutions with increasing resolution ({\em{resolution}} refers to {\em{horizontal resolution}} hereafter). Although some characteristics of resolution sensitivity are model dependent, others are common across different AGCMs and indicate a common mechanism. For example, increasing resolution lends itself to greater stratiform precipitation and reduced parameterized convective precipitation in the Hadley Centre climate model \citep[HadAM3;][]{PS2002CD}, the Max Planck Institute for Meteorology model \citep[ECHAM5;][]{HETAL2006JC} and the National Center for Atmospheric Research (NCAR) Community Climate Model \citep[CCM2;][see Figure~\ref{fig:cam-history}]{WETAL1995CD}. So while the purpose of this study is to provide a process-level understanding of resolution sensitivity in the NCAR models, these results are useful for understanding resolution sensitivity in models that exhibit similar behavior.}}
...
\end{equation}
where $\Delta t_{phys,0}$ is taken to be the standard $1800$ s used in CAM-SE-CSLAM for the standard climate resolution, $n_{e,0} = 30$ (equivalent to an average equatorial grid spacing $\Delta x = 111.2$km). This scaling was chosen to avoid large time-truncation errors in a rising moist bubble test \citep[Appendix A in][]{HETAL2019JAMES}, and it is understood that this choice of $\Delta t_{phys}$ will likely lead to greater resolution sensitivity \citep{W2008TELLUS}. {\color{red}{Due to the large sensitivity of vertical velocity to $\Delta t_{phys}$ \citep{HR2018JAMES}, the lack of scientific consensus on the numerical and physical justifications for how $\Delta t_{phys}$ should vary with resolution remains a severe limitation to characterizing vertical velocities across resolutions in AGCMs.}} The convective time-scale in the deep convection scheme is fixed at 3600 s in all simulations.
...
{\color{red}{The HadAM3 and ECHAM5 models both show a reduction in convective precipitation rates with increasing resolution (Figure~\ref{fig:cam-history}). As with CAM, both models use a stability based closures in their deep convection schemes \citep{GR1990MWR,N1994ECMWF} and both show an increase in tropospheric temperatures with resolution \citep{PS2002CD,RETAL2006JC}. In as much as the increase in stability with resolution in these models are driven by greater subsidence warming, the mechanism suppressing parameterized convective activity with resolution in CAM may also explain the resolution sensitivity in HadAM3 and ECHAM5.}}
