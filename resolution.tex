...
A curious, strong negative correlation is discovered between the climatological, global mean activity of the \cite{ZM1995AO} deep convection scheme (ZM scheme) and global mean subsiding motion in the simulations. Since the ZM scheme is modulated by an entraining plume calculation, referred to as the dilute convective available potential energy (dilute CAPE), the authors sought to understand the influence of subsidence on dilute CAPE. In the deep tropics ($\pm 10^{\circ}$ latitude), subsiding regions have much smaller dilute CAPE values compared to ascending regions due to the influence of the drier, subsiding environment on dilute CAPE, and subsidence is a good predictor of the activity of the ZM scheme within the simulations. With increasing resolution and in the deep tropics, the occurrence of subsiding motion increases dramatically and at the expense of ascending motion, and the activity of the the ZM scheme decreases overall. This is not completely unexpected due to the $\Delta x^{-1}$ scaling of the vertical velocities, e.g., a doubling of resolution can generate the same mass flux in just half the area. The increased efficiency of upward mass fluxes with resolution suggests that the occurrence of ascending motion may decrease with resolution, while still maintaining statistical equilibrium with the large-scale forcing.

{\color{red}{EVERYTHING BELOW HERE NEEDS SOME WORK}}

In addition to the change in balance of ascending/descending motion in the deep tropics, there is also a decrease in dilute CAPE of both ascending/descending regimes with increasing resolution. A sensitivity analysis of the dilute CAPE calculation has verified that this change in the background stability of the deep tropics is a result of temperature changes with resolution. A similar change in background stability of the environment occurs everywhere else in the simulations, equator-to-pole, and reduces the activity of the ZM scheme globally. The increased stability is a result of adiabatic warming due to increasing subsidence with resolution. Subsidence warming falls out of the component of the dilute CAPE budget due to the advection of dry static energy and moisture by the environment, referred to as dCAPE \citep{Z2002JGR}. Subsidence warming arises from the vertical advection of dry static energy which simplifies to $\frac{w}{c_{pd}} \frac{\partial gz}{\partial z} = \frac{g}{c_{pd}} w$, and directly proportional to the vertical velocity $w$ by the factor $\frac{g}{c_{pd}}$, with $g$ the acceleration of gravity and $c_{pd}$ the specific heat capacity of dry air (F. Song, personal communication). The magnitude of downward $w$ increases everywhere in the model with increasing resolution, reducing dilute CAPE everywhere in the model with increasing resolution.

The subtropics ($\pm (10^{\circ}-10^{\circ})$ latitude) are uniquely less sensitive to subsiding motion, and a poor predictor of the activity of the ZM scheme in general. This is likely due to the $\pm (10^{\circ}-10^{\circ})$ latitude region coinciding with a global maximum in surface latent heat fluxes, which increases dilute CAPE above the threshold for convection, despite appreciable subsiding motion aloft opposing the growth of dilute CAPE. These opposing influences result in small values of dilute CAPE, and the ZM scheme acts more like a shallow convection scheme, producing copious amounts of drizzle. AGCMs are known to suffer from an excess of drizzle relative to observations \citep{D2006JCLIM}. \cite{XETAL2019JAMES} recently proposed a new convective trigger for the ZM scheme containing an additional criterion for convection, that dCAPE must be positive. Based on the analysis in this study, it is reasonable to assume that dCAPE would be negative in drizzling regions and convection would not occur. The authors suggest that the \cite{XETAL2019JAMES} be explored as a means to mitigate the drizzling bias in CAM.

The Intertropical Convergence Zone (ITCZ) changes with increasing resolution, with proportionally more precipitation generated by the the stratiform scheme relative to the ZM scheme. Energetically, one might assume that stratiform precipitation rates are increasing with resolution to compensate for the reduction in ZM precipitation rates to satisfy globally average radiative-convective equilibrium. But from a process point of view, it is shown that resolved upward moisture fluxes are highly correlated with stratiform precipitation rates, and that there is an increase in both upward moisture fluxes and stratiform precipitation with resolution. Through decomposing the stratiform precipitation in the ITCZ region into the components of the moisture flux, it is shown that larger magnitude vertical velocities are associated with larger stratiform precipitation rates, and these large precipitation rates increasingly contribute to the increase in the time mean stratiform precipitation rate with resolution.
