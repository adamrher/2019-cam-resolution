...
This study analyzes a convergence experiment in an aqua-planet configuration using the Community Atmosphere Model (CAM), with the spectral-element dynamical core option coupled to the accelerated multi-tracer transport scheme (CAM-SE-CSLAM), and version 6 physics (CAM6), in order to understand longstanding convergence issues that have persisted within the CAM lineage since the models' inception (see Figure~\ref{fig:cam-history}). Establishing a complete understanding of resolution sensitivity in CAM and AGCMs alike is crucial, since inevitable responses to native grid resolution cannot be ignored in the pursuit of scale-aware physics. As with most versions of CAM, in the CAM6 convergence experiment the atmosphere becomes drier and less cloudy with increasing resolution, and convective precipitation decreases while stratiform precipitation increases with resolution. Prior studies have established that the drying and reduction in cloudiness is a result of increases in the magnitude of vertical velocities with resolution, which more efficiently advect dry air aloft, downward \citep{KW1991JGR,WETAL1995CD,YETAL2014JCLIM,HR2017JCLIM}. This study aims to then understand why and how vertical velocities change with resolution, and why the convection precipitation decreases at the expense of stratiform precipitation with increasing resolution.

The vertical velocities are found to fit a power law scaling $\Delta x^n$ with $\Delta x$ the grid spacing and $n=-1$. The power law exponent $n=-1$ is derived from a scale analysis of the Poisson equation, indicating that the pressure gradients scale like the inverse of the horizontal scale of buoyancy perturbations, facilitating convergence into an air column and driving vertical motion. The horizontal scale of buoyancy perturbations in CAM have been previously shown to result from stratiform cloud formation, which collapse to the smallest resolvable feature in the model \citep{HR2018JAMES}, known as the effective resolution \citep{S2011LNCSE}. The horizontal scale of stratiform clouds is then proportional to $\Delta x$, which results in the $n=-1$ power law exponent. This scaling is consistent with studies describing the resolution sensitivity of large-eddy simulations, which use similar physical arguments, but adapted for non-hydrostatic scales \citep{WETAL1997MWR,PG2006JAS,JR2016QJRMS}.

The $n=-1$ power law scaling is potentially in conflict with the scaling proposed by \cite{RETAL2016CD}, $n=h-1$, with $h$ being half the exponent of the second-order structure function $\Delta x^{2h}$ of the horizontal wind. \cite{RETAL2016CD} provide evidence for $h<1$ in their regional climate model simulations, but propose that $h=\frac{1}{3}$ is an emergent constraint for $n=-\frac{2}{3}$, based on observations. This is an intriguing proposal and the authors don't seek to dismiss its potential value. But it is suggested that the overwhelming support for the $h=\frac{1}{3}$ exponent in the second-order structure function is only relevant for horizontal scales on the order of $100$ km and less \citep{L1999JFM,CL2001JGR}, which is not resolved in even our highest resolution simulation $\Delta x=27.8$ km since that CAM-SE-CSLAM has an effective resolution in the range of $5-10\Delta x$ \citep{HETAL2019JAMES}. The nonexistence of $h=\frac{1}{3}$ in the $\Delta x=27.8$ km simulation is verified through analyzing the slope of kinetic energy spectrum $-\beta$, which is in duality with the second-order structure function through the Weiner–Khinchine theorem. For $h=\frac{1}{3}$, $\beta=-\frac{5}{3}$, which is not observed in the simulation {\color{red}{(Figure~X)}}.

A curious, strong negative correlation is discovered between the climatological, global mean activity of the \cite{ZM1995AO} deep convection scheme (ZM scheme) and global mean subsiding motion in the simulations. Since the ZM scheme is modulated by an entraining plume calculation, referred to as the dilute convective available potential energy (dilute CAPE), the authors sought to understand the influence of subsidence on dilute CAPE. In the deep tropics ($\pm 10^{\circ}$ latitude), subsiding regions have much smaller dilute CAPE values compared with ascending regions due to the influence of the drier, subsiding environment on dilute CAPE, and subsidence is a good predictor of the activity of the ZM scheme within in the simulations. With increasing resolution and in the deep tropics, the occurrence of subsiding motion increases dramatically and at the expense of ascending motion, and the activity of the the ZM scheme decreases overall. This is not completely unexpected since e.g., for a doubling of the resolution, and the power-law $\Delta x^{-1}$ scaling of the vertical velocities, one can simulate the same mass fluxes in half the area. The increased efficiency of upward mass fluxes with resolution suggests that the occurrence of ascending motion may decrease with resolution, while still maintaining a statistical equilibrium with the large-scale forcing.

In addition to the change in balance of ascending/descending motion in the deep tropics, there is also a decrease in dilute CAPE of both ascending/descending regimes with increasing resolution. A sensitivity analysis of the dilute CAPE calculation has verified that this change in the background stability of the deep tropics is a result temperature changes with resolution. A similar change in background stability of the environment occurs everywhere else in the simulations, equator-to-pole. 

The relationship between subsidence and temperature falls out of the CAPE equation, which is closely related to dilute CAPE. The CAPE budget can be separated into two components \citep{Z2002JGR}; instability due to the thermodynamic state of parcels in the boundary layer and the instability generated through radiative cooling and advection of dry static energy and moisture by the environment, i.e., the resolved flow. The latter term is defined as, 
\begin{equation}
...
Unlike the deep tropics, dilute CAPE in the subtropics ($\pm (10^{\circ}-10^{\circ})$ latitude) is less dependent on subsiding motion, and a poor predictor of the activity of the ZM scheme in general. This is likely due to the $\pm (10^{\circ}-10^{\circ})$ latitude region coinciding with a global maximum in surface latent heat fluxes, which increases dilute CAPE above the threshold for convection, and despite descending motion aloft opposing the growth of dilute CAPE. These opposing influences results in quite small values of dilute CAPE, with ZM scheme acting more like a shallow convection scheme, and producing copious amounts of drizzle. \cite{XETAL2019JAMES} recently proposed a new convective trigger the ZM scheme that avoids this excess drizzle in the model.
