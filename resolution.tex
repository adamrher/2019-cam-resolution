...
This study analyzes a convergence experiment in an aqua-planet configuration using the Community Atmosphere Model (CAM), with the spectral-element dynamical core option coupled to the accelerated multi-tracer transport scheme (CAM-SE-CSLAM), and version 6 physics (CAM6), in order to understand longstanding convergence issues that have persisted within the CAM lineage since its inception some 30 years ago. Establishing a complete understanding of resolution sensitivity in CAM and AGCMs alike is crucial, since inevitable responses to native grid resolution cannot be ignored in the pursuit of scale-aware physics. As with most versions within the CAM lineage (see Figure~\ref{fig:cam-history}), the atmosphere becomes drier and less cloudy with increasing resolution, and convective precipitation decreases while stratiform precipitation increases with resolution. Prior studies have established that the drying and reduction in cloudiness is a result of increases in magnitude of vertical velocities with resolution, which more efficiently advect dry air downward \citep{KW1991JGR,WETAL1995CD,YETAL2014JCLIM,HR2017JCLIM}. This study aims to then understand why and how vertical velocities change with resolution, and why the convection precipitation decreases at the expense of stratiform precipitation with increasing resolution.

The vertical velocities were found to fit a power law scaling $\Delta x^n$ with $n=-1$. The scaling is derived from a scale analysis of the Poisson equation in which the pressure gradients scale like the inverse of the horizontal scale of buoyancy perturbations, facilitating convergence into an air column and driving vertical motion. The horizontal scale of buoyancy perturbations in CAM have been previously shown to result from stratiform cloud formation, which collapse to the smallest resolvable feature in the model \citep{HR2018JAMES}, known as the effective resolution \citep{S2011LNCSE}. The horizontal scale of stratiform clouds is then proportional to $\Delta x$, which results in the $n=-1$ power law exponent. This scaling is consistent with studies describing the resolution sensitivity of large-eddy simulations, which use similar physical arguments, but adapted for non-hydrostatic scales \citep{WETAL1997MWR,PG2006JAS,JR2016QJRMS}.

The $n=-1$ power law scaling is potentially in conflict with the scaling proposed in \cite{RETAL2016CD}, $n=h-1$, with $h$ being half the exponent of the second structure function $\Delta x^{2h}$ of the horizontal wind. \cite{RETAL2016CD} provide evidence for $h<1$ in their regional climate model simulations, but propose that $h=\frac{1}{3}$ is an emergent constraint for $n=-\frac{2}{3}$, based on observations and models. This is intriguing proposal and the authors don't dismiss it's potential value. But rather, it is proposed that the overwhelming support for the $h=\frac{1}{3}$ exponent in second-order structure function is only relevant for horizontal scales on the order of $100$ km and less, which is only marginally resolved in even our highest resolution simulation, with an average equatorial grid spacing of $\Delta x=27.8$ km. But it is the effective resolution that determines the resolvable scales of motion, which is in the range of $5-10\Delta x$ in CAM-SE-CSLAM \citep{HETAL2019JAMES}. 

The Weiner–Khinchine theorem provides a duality between the second-order structure function and the slope of kinetic energy spectrum $-\beta$, in which $h=\frac{1}{3}$ gives $\beta=\frac{5}{3}$, which has robust support from observations and models at the mesoscale. The simulations used in this paper are too coarse to resolve the $\beta=\frac{5}{3}$ slope, and has only been observed in the CAM model with $\Delta x=13.9$ km. In this study, $\beta=3$ near the effective resolution of the model, and while tempting to apply the Weiner–Khinchine theorem to derive a value for $h$, it is only valid in the range $1<\beta<3$; $\beta=3$ violates stationarity \citep{DETAL1996JAS}. One could however, compute the second-order structure function in the simulations directly to estimate a value for $h$ in the simulations, but this is beyond the scope of the current study. 
